...
def min_quad(Q, b, Delta, epsilon, max_iter = 1000):
    ...
    while np.linalg.norm(p(u, Q, b)) > Delta and iter < max_iter:
        ...
        iter += 1
    if iter >= max_iter:
        raise ValueError("The algorithm did not converge in the expansion phase")
    ...
    while True and iter < max_iter:
        ...
        if np.linalg.norm(p_prime) <= Delta and np.linalg.norm(np.linalg.norm(p_prime) - Delta) < epsilon:
        ...
        iter += 1
    if iter >= max_iter:
        raise ValueError("The algorithm did not converge in the bisection phase")
...
def check_kkt(x_star, Q, b, Delta, atol):
    ...
    def calc_lambda_star(x_star, Q, b, Delta):
        ...
        if np.isclose(np.linalg.norm(x_star), Delta):
            lambda_star = -np.dot(grad_f_at_x_star, grad_h_at_x_star) / np.dot(grad_h_at_x_star, grad_h_at_x_star)
        else:
            lambda_star = 0
        return lambda_star
    lambda_star = calc_lambda_star(x_star, Q, b, Delta)
    stationarity = np.allclose(df(x_star, Q, b) + lambda_star * grad_constraint(x_star), 0, atol=atol)
    primal_feasibility = np.linalg.norm(x_star) <= Delta
    dual_feasibility = lambda_star >= 0
    complementary_slackness = np.isclose(lambda_star * norm_constraint(x_star, Delta), 0, atol=atol)
    if not stationarity:
        print(f"f'(x_star) + lambda_star * grad_constraint(x_star) = {df(x_star, Q, b) + lambda_star * grad_constraint(x_star)}")
    ...
    if not complementary_slackness:
        print(f"lambda_star * norm_constraint(x_star, Delta) = {lambda_star * norm_constraint(x_star, Delta)}")
    else:
        return True